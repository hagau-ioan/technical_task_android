...
class StatsImpl @Inject constructor(@ApplicationContext private val context: Context) : Stats {
    ...
    override fun sendUIEvent(event: StatsEventsUI) {
        event.eventName?.let {
            // Based on this WHEN we can filter some "events" so we can send different events
            // to MixPanel and others to Firebase, etc...
//            when (event) {
//                is StatsEventsUI.ButtonClicked -> {
//                }
//                is StatsEventsUI.ScreenPageOpened -> {
//                }
//            }
//            FacadeAnalyticsTelemetryStats.getFirebaseAnalytics(context).logEvent(it) {
//                param(FirebaseAnalytics.Param.LOCATION, event.value)
//            }
        }
    }
